  ...
  return (
    <div className="rounded-3xl bg-brand-800 p-6 text-white shadow-sm sm:p-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-brand-100">Willowbrook Rewards</p>
          <p className="mt-2 text-4xl font-bold">{balance.toLocaleString()} <span className="text-lg font-medium text-brand-100">pts</span></p>
          <p className="mt-1 text-sm text-brand-100">{lifetimePoints.toLocaleString()} lifetime points earned</p>
        </div>
      ...
      <div className="mt-6">
        {nextTier ? (
          ...
          <p className="text-xs text-brand-100">You've reached our top tier — thank you for being a Best Friend member.</p>
        )}
      </div>
